...
class TOPCharacterChart extends StatelessWidget {
  final List<Top5Characters> topcharactervotes;
  const TOPCharacterChart({super.key, required this.topcharactervotes});

  @override
  Widget build(BuildContext context) {
    BarData data = BarData(
      data: topcharactervotes,
    );
...
//getting bottom widget
  Widget getbottomTitles(double value, TitleMeta meta) {
    final int index = value.toInt() % topcharactervotes.length;
    const style = TextStyle(color: Colors.grey, fontSize: 14, fontWeight: FontWeight.normal);
    return SideTitleWidget(
        axisSide: meta.axisSide, child: Text(topcharactervotes[index].character!, style: style));
  }
