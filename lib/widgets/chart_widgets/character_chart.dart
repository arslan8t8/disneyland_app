...
class CharacterChart extends StatelessWidget {
  final List<CharacterVotes> charactervotes;
  const CharacterChart({super.key, required this.charactervotes});
...
//getting bottom widget
  Widget getbottomTitles(double value, TitleMeta meta) {
    final int index = value.toInt() % charactervotes.length;
    const style = TextStyle(color: Colors.grey, fontSize: 14, fontWeight: FontWeight.normal);
    return SideTitleWidget(
        axisSide: meta.axisSide, child: Text(charactervotes[index].character!, style: style));
  }
